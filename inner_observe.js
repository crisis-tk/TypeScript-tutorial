    ...
    let observable = rxjs_1.Observable.create((observer) => {
        try {
            ...
            array.forEach((element) => {
                if (typeof (element) === 'number' && element > 30)
                    throw "数が大きすぎます";
...
// 自作のfrom関数に配列を渡してみる。
let observa = my_from([10, 20, 30, 40, 50]);
// 丁寧に３つの関数をsubscribeに渡してみる例
observa.subscribe(value => console.log(value), // データ到着時に呼ばれる関数。next関数
// データ到着時に呼ばれる関数。next関数
